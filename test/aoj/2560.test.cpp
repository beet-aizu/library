...
signed main(){
  ...
  Convolution2D<int, decltype(tran1)> conv1(tran1);
  Convolution2D<int, decltype(tran2)> conv2(tran2);
  auto mul1=[&](auto a,auto b){return (ll)a*b%ntt1.md;};
  auto mul2=[&](auto a,auto b){return (ll)a*b%ntt2.md;};

  auto cs1=conv1.multiply(as,bs,mul1);
  auto cs2=conv2.multiply(as,bs,mul2);
