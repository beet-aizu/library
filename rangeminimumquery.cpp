...
struct RMQ{
  ...
  const int ms = sz-1;
  int n;
  vector<int> P,L,R,D,E,A,B,T,ht;
  vector<vector<int> > dat;
  RMQ(int n):
    n(n),P(n,-1),L(n,-1),R(n,-1),D(n),
    E(n*2,0),A(n*2,-1),B(n*2/lg+1),T(sz,0){}

  void dfs(int v,int d,int &k){
    D[v]=k;
    A[k]=P[v];
    E[k++]=d;
    if(~L[v]) dfs(L[v],d+1,k);
    if(~R[v]) dfs(R[v],d+1,k);
    A[k]=P[v];
    E[k++]=d-1;
  }
